...
- [X] T001 Initialize the Nx workspace at repo root: create `nx.json`, root `package.json` (workspace/`workspaces` config, `engines.node` = 22, single `npm install` entry), and commit the generated `package-lock.json`
- [X] T002 [P] Create `tsconfig.base.json` at repo root with shared compiler options and the `@workboard/shared` path alias
- [X] T003 [P] Configure repo-root linting/formatting: `eslint.config.mjs` (flat config) and `.prettierrc`
- [X] T004 [P] Add `.gitignore` (node_modules, dist, `.env`, `cdk.out`, `src-tauri/target`) and `.nvmrc` (Node 22) at repo root
- [X] T005 [P] Configure Conventional Commits + SemVer tooling: `commitlint.config.js` and a commit-msg hook (husky) at repo root (FR-016)
- [X] T006 [P] Create `README.md` skeleton at repo root with the required-toolchain/versions section (FR-015)
...
- [X] T007 Configure Nx `targetDefaults` in `nx.json` (build/lint/test/serve caching, `dependsOn: ["^build"]`) so every registered package is covered by workspace-wide runs (FR-001, SC-002)
- [X] T008 [P] Add a shared Vitest base preset at `tools/vitest/vitest.base.ts` (and register `vitest.workspace.ts` at repo root) reusable by all packages
- [X] T009 [P] Add `.env.example` at repo root and document the env/secret convention (no secrets committed or bundled) in `README.md` (FR-017)
...
- [X] T010 [P] [US1] Add sample Vitest test validating the shared Zod schemas in `libs/shared/src/schemas/task.spec.ts` (proves the test layer is wired; run by `nx run-many -t test`)

### Implementation for User Story 1

- [X] T011 [P] [US1] Scaffold the `libs/shared` package: `libs/shared/project.json` (build/lint/test targets), `libs/shared/tsconfig.json`, `libs/shared/src/index.ts`
- [X] T012 [P] [US1] Define Zod schemas + inferred TypeScript types for Task, Project, Note, and User/Account in `libs/shared/src/schemas/{task,project,note,user}.ts` per data-model.md
- [X] T013 [US1] Export all schemas/types via the barrel `libs/shared/src/index.ts` (depends on T012)
- [X] T014 [US1] Verify workspace-wide `nx run-many -t build lint test` and `nx affected` behavior against the nx-workspace-contract in `nx.json` (registration coverage, no bespoke scripts)
- [X] T015 [US1] Document the single install command and workspace-wide build/lint/test commands + package-registration behavior in `README.md` (FR-003, FR-015)
...
- [X] T016 [P] [US2] Add sample Vitest component test asserting the shell renders the four nav areas (Week/Projects/Notes/Overview) in `apps/frontend/src/app/app-shell.spec.tsx`

### Implementation for User Story 2

- [X] T017 [US2] Generate the `apps/frontend` React + Vite app via Nx with build/serve/lint/test targets in `apps/frontend/project.json`
- [X] T018 [P] [US2] Configure Tailwind + shadcn/ui base design system under `apps/frontend/src/components/ui/` and `apps/frontend/tailwind.config.ts`
- [X] T019 [US2] Configure `vite-plugin-pwa` (manifest + service worker) in `apps/frontend/vite.config.ts` and add PWA manifest/icons in `apps/frontend/public/`
- [X] T020 [P] [US2] Define the platform adapter interface + web implementation in `apps/frontend/src/platform/` (single interface, no per-target fork)
- [X] T021 [US2] Add the Tauri 2 desktop shell in `apps/frontend/src-tauri/` (+ tauri adapter impl) and a `tauri` target in `apps/frontend/project.json`
- [X] T022 [US2] Build the responsive app shell with a left sidebar of Week/Projects/Notes/Overview placeholders from the shared design system in `apps/frontend/src/app/`
...
- [X] T023 [P] [US3] Add sample Vitest integration test for `GET /health` covering healthy (200) and persistence-unreachable (503) cases in `apps/backend/src/routes/health.spec.ts` per contracts/health-api.md

### Implementation for User Story 3

- [X] T024 [US3] Generate the `apps/backend` Nx Node/Express app with build/serve/lint/test targets in `apps/backend/project.json`
- [X] T025 [P] [US3] Create the mandated layer directories `routes/`, `controllers/`, `services/`, `repositories/`, `domain/`, `middleware/`, `validation/`, `shared/` under `apps/backend/src/`
- [X] T026 [P] [US3] Create empty feature-module placeholders `modules/{auth,tasks,projects,notes,overview}` (with `.gitkeep`) under `apps/backend/src/modules/`
- [X] T027 [US3] Create the Express app factory and both entries: `apps/backend/src/app.ts`, `apps/backend/src/main.ts` (local listen), and `apps/backend/src/lambda.ts` (`@codegenie/serverless-express` handler) sharing one `app.ts` (FR-008)
- [X] T028 [US3] Implement the DynamoDB repository abstraction + connectivity probe using `@aws-sdk/lib-dynamodb` in `apps/backend/src/repositories/health.repository.ts` (FR-011)
- [X] T029 [US3] Implement the health flow through the layers: `apps/backend/src/routes/health.routes.ts` → `controllers/health.controller.ts` → `services/health.service.ts` (returns healthy/unhealthy per persistence, no logic in route/controller)
- [X] T030 [P] [US3] Add a DynamoDB Local `docker-compose.yml` for backend/health dev in `apps/backend/`
...
- [X] T031 [P] [US4] Add a sample Vitest CDK assertion test verifying the synthesized template contains the DynamoDB table, Lambda, API Gateway, S3/CloudFront, and Cognito resources in `apps/infra/lib/stack.spec.ts`

### Implementation for User Story 4

- [X] T032 [US4] Generate the `apps/infra` AWS CDK v2 app with build/lint/test + `synth`/`deploy` targets in `apps/infra/project.json` and the CDK entry in `apps/infra/bin/infra.ts`
- [X] T033 [P] [US4] Define the DynamoDB single table `WorkBoard` (PK/SK) in `apps/infra/lib/data-stack.ts` per data-model.md
- [X] T034 [P] [US4] Define the Cognito user pool in `apps/infra/lib/auth-stack.ts`
- [X] T035 [US4] Define the single Lambda (packaging the backend) + API Gateway in `apps/infra/lib/api-stack.ts` (depends on T027)
- [X] T036 [US4] Define S3 + CloudFront static hosting for the frontend build in `apps/infra/lib/web-stack.ts`
- [X] T037 [US4] Wire stack outputs (API base URL, CloudFront URL, table name, user-pool IDs) in `apps/infra/lib/` (consumed via env, not committed)
...
- [X] T038 [US5] Add the sample Playwright e2e asserting the shell renders the four nav areas in `apps/frontend-e2e/src/app-shell.e2e.ts`

### Implementation for User Story 5

- [X] T039 [US5] Scaffold the `apps/frontend-e2e` Playwright project with `playwright.config.ts`, an `e2e` target in `apps/frontend-e2e/project.json`
- [X] T040 [US5] Create the GitHub Actions CI workflow running `nx affected -t lint test` + Playwright on every PR in `.github/workflows/ci.yml` (FR-012)
- [X] T041 [P] [US5] Document `main` branch protection (PR required, all gates must pass to merge, no direct pushes) in `README.md`/repo docs (FR-013)
...
- [X] T042 [P] Complete `README.md` with build/run/test/deploy commands for every package (frontend, backend, infra, shared) sufficient for a new contributor (FR-015, SC-008)
- [X] T043 Run all five quickstart.md validation scenarios end to end and record results
- [X] T044 [P] Final scope/security check: confirm no secrets committed or bundled (FR-017) and no end-user feature behavior was added (FR-018)
